...
2. **Choose a data folder** for the CSV “database”. You have two options:

   **A) Environment variable (recommended)**  
   Set `UMBRELLA_DATA_DIR` to the folder you want.

   - **Windows (PowerShell, current terminal only):**
     ```powershell
     $env:UMBRELLA_DATA_DIR="C:\Users\zone0\Desktop\Homework\INT6127\Data_csv"
     ```
     To persist for future terminals:
     ```powershell
     setx UMBRELLA_DATA_DIR "C:\Users\zone0\Desktop\Homework\INT6127\Data_csv"
     ```
     (Restart VS Code after `setx`.)

   - **macOS/Linux (bash/zsh):**
     ```bash
     export UMBRELLA_DATA_DIR="$HOME/umbrella_data"
     ```

   **B) Edit the constant in code**  
   The script contains:
   ```python
   ...
   ```
   Change the default path to your desired folder.
...
3. **Run the app**
   ...
   - **Terminal**:
     ```bash
     # Windows
     python campus_rental_csv_gui.py
     # or, if you saved as umbrella_app.py:
